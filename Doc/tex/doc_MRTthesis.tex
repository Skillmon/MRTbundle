...
\begin{describeopt}{tikzunderline, tUline}
  ...
  in \autoref{sec:thesis:mac}.
\end{describeopt}
\begin{describeopt}{UKenglish}
  If used the document will be using the \opt{UKenglish} definition of
  \pkg{babel}. Many strings used in the package will be in English, but some
  might be missed out. If you find any of which you think they should be
  translated, please contact me as described in \autoref{sec:bugs}.
\end{describeopt}
...
\begin{describemacro}{WUline}[\oarg{height}\marg{text}]
  ...
  default in math mode \texttt{\csuse{MRTthesis@WUline@mathheight}} is used, in
  text mode \texttt{\csuse{MRTthesis@WUline@textheight}}.
\end{describemacro}
\begin{describemacro}{tUline}%
  ...
  use the class option \opt{tUline}.\\[\parskip]
  ...
  assumed that the lines are equally separated with \cs{baselineskip}. It needs
...
\begin{multicols}{2}%>>>
  \begin{itemize}[leftmargin=10pt]
    ...
    \item \pkg{lmodern}
    \item \pkg{helvet}
    \item \pkg{babel} [ngerman] or if \opt{UKenglish} is used with
    ...
    \item if the \opt{minimal} option is not used:
      \begin{itemize}
        \item \pkg{siunitx}
          ...
          If \pkg{babel}'s \opt{UKenglish} is used [locale=UK] will be used, if
          \opt{ngerman} [locale=DE].
        \item \pkg{biblatex}
          [%
