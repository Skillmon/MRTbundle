...
\begin{describeopt}{env}[\meta{name}]%>>>
  Uses the tabular like environment \meta{name} instead of \env{MRTtabular}.
\end{describeopt}%<<<
...
\begin{describeopt}{head rows, head}[\meta{num}]%>>>
  ...
  different from a stand alone \env{MRTtabular} which by defaults to \texttt{0}
  rows.
\end{describeopt}%<<<
...
\section{A very long caption that has to be line broken because it is way to
long for a single line in the resulting document.}
