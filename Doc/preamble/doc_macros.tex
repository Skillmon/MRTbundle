...
\int_set:Nn \l_docMRTbundle_fakeit_level_int { -\c_one }
...
\NewDocumentCommand \docIndividualVersions {}%>>=
  {
    ...
    \begin{MRTtable}
      [
        ,col=lcc
        ,float=false
        ,caption={Versions~of~Individual~Packages}
        ,pre=
          \setbox0\hbox{0.0.00}%
          \edef\versionline##1%
            {
              \noexpand\pkg{MRT##1}
              \noexpand& \noexpand\csuse{MRT##1@date}
              \noexpand& \noexpand\makebox[\the\wd0][l]{\noexpand\csuse{MRT##1@version}}
              \noexpand\\
            }
      ]
    ...
    \scalebox{#1}
      {
        \makebox[1em][c]
          {
            \makebox[0pt][c]{\raisebox{.17ex}{\scalebox{.6}{\kern-.11ex!}}}
...
\NewDocumentEnvironment { incompatibilities } { m }%>>=
  {%>>=
    ...
    \dim_set:Nn \l_tmpa_dim
      {
        \linewidth - \box_wd:N \l_tmpa_box - \box_wd:N \l_tmpb_box - 8\tabcolsep
      }
    %\exp_args:Nnx
    \setstretch { 1.408 }
    \begin{ longtable }
      {
        ...
        >{ \raggedright \setstretch { 1 } } p { .3 \l_tmpa_dim }
        >{ \setstretch { 1 } } p { .7 \l_tmpa_dim }
      }
    \caption{Downwards~Incompatibilities~in~#1~introduced~by~Date~and~Version}\\
    \headS
    ...
    \caption[]
      {%
        Downwards~Incompatibilities~in~#1~introduced~by~Date~and~Version~
        (\textit{continued})%
      }\\
    \headS
    Date & Version & Affected & Description \\
...
\NewDocumentCommand \DoAndPrint { +v }%>>=
  {
    ...
    \group_begin:
      ...
      \exp_after:wN
    \group_end:
    \exp_after:wN \__docMRTbundle_scantokens:w
      \exp_after:wN { \l__docMRTbundle_doandprint_tl }
    \tl_set:Nn \l__docMRTbundle_doandprint_tl { #1 }
    \__docMRTbundle_replace_verb_newline:Nn
